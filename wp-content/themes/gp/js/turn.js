...
/*globals $, jQuery, window, document, navigator */
...
(function () {
	$.fn.fold = function (opts) {
		var ie6, ie55, options, turn_hideme, turn_wrapper, turn_object, img,
		...
		ie55 = (navigator.appName === "Microsoft Internet Explorer" && parseInt(navigator.appVersion, 10) === 4 && navigator.appVersion.indexOf("MSIE 5.5") !== -1);
		ie6 = (navigator.appName === "Microsoft Internet Explorer" && parseInt(navigator.appVersion, 10) === 4 && navigator.appVersion.indexOf("MSIE 6.0") !== -1);
		...
		turn_hideme = $('<div id="turn_hideme">');
		turn_wrapper = $('<div id="turn_wrapper">');
		turn_object = $('<div id="turn_object">');
		img = $('<img id="turn_fold" src="http://goodbyepicasso.com/wp-content/themes/gp/images/curl/' + 
			options.turnImage + '" />');
		...
		// Thanks to @zzzrByte for this bit!
			turn_wrapper.click(function () {
				...
				}, options.speed || 1500, function () {
					if (options.callback) {
						options.callback(function () {
							turn_object.removeAttr('style').css({
								width: options.startingWidth, 
								height: options.startingHeight
							});
						});
